...
ENUM_VALUE_RE = re.compile(r"\b[A-Z]{3,}(?:_[A-Z]+)*\b")
...
def _query_bigrams(query: str) -> set[str]:
    """Generic two-word phrase overlap, not tied to any hardcoded topic list.

    Complements the hardcoded _query_phrases below: this catches meaningful
    multi-word overlap (e.g. "subscription state", "review state") for
    topics we haven't explicitly special-cased.
    """
    tokens = [
        token
        for token in re.findall(r"[a-z0-9]+", query.lower())
...
def _asks_for_state_or_status(query: str) -> bool:
    """Generic detector for "what state/status is X in" style questions.

    Unlike _asks_for_graphql_status (which is specifically about GraphQL
    HTTP status codes), this covers any workflow-state question -
    subscriptions, contracts, job postings, whatever. It doesn't know the
    answer ahead of time, it just recognizes the question shape so we can
    boost chunks containing concrete enum-like values.
    """
    normalized = query.lower()
    return any(
        phrase in normalized
        for phrase in ("state", "status", "stage", "workflow")
    )
...
def _rerank_score(query: str, doc: Document) -> int:
    ...
    # Generic bigram overlap - not tied to a fixed phrase list, so it helps
    # on topics we haven't explicitly coded a boost for.
    score += 4 * sum(1 for bigram in bigrams if bigram in text)
    ...
    if _asks_for_state_or_status(query):
        # Boost chunks that actually contain a concrete enum-looking value
        # (ALL CAPS token) - generic signal that this chunk states a real
        # state/status name rather than just discussing the concept.
        enum_hits = len(ENUM_VALUE_RE.findall(raw_text))
        score += 6 * min(enum_hits, 3)
